...
void ExceptionHandler(ExceptionType exceptiontype, int vaddr)
{
	...
	case SYSCALL_EXCEPTION: {
		...
		case SC_COND_CREATE:{
			...
			int32_t condid = g_object_ids->AddObject(cond);
    ...
    case SC_MMAP : {
      DEBUG('e', (char*)"MMAP syscall called\n");
      int32_t  map= (int32_t) g_machine->ReadIntRegister(4);
      int size = g_machine->ReadIntRegister(5);
      OpenFile *mappedFile = (OpenFile *)g_object_ids->SearchObject(map);
      if ((mappedFile == NULL) || (mappedFile->typeId != FILE_TYPE_ID)) {
        g_machine->WriteIntRegister(2, -1);
