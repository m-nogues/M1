...
void routine_envoi(char *s, int taille){
	...
	if(nb_envoi != taille+1){
		n_printf("failled nb_envoi = %d/%d \n",nb_envoi, taille);
		Exit(1);
	}
		n_printf("succes envoie de la chaine \"%s\"\n", s);
}

void fullChar(){
	routine_envoi("defghijklmopqrstuvwxyzabcdefghijklmopqrstuvwxyzabcdefghijklmopqrstuvwxyzabcdefghijklmopqrstuvwxyzabcdefghijklmopqrstuvwxyzabcdefghijklmopqrstuv", 256);
	Exit(0);
}
