...
int main (){
  ...
  if((thread1 = threadCreate(threadName,routine_envoi))==-1){
    PError("creation thread 1 failled");
  }
  n_printf("Test thread1 passed\n");
  if((thread2 = threadCreate(threadName,routine_envoi))==-1){
    PError("creation thread 2 failled");
  }
  n_printf("Test thread2 passed\n");

  if((thread3 = threadCreate(threadName,routine_envoi))==-1){
