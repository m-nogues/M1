...
public class MIPSCodeGenerator implements MIPSCGConstants {
	...
	/**
	 * Instantiates a new MIPS code generator.
	 *
	 * @param out
	 *            the out
	 */
	...
	public Code3a addStubMain(Code3a c) {
		...
		FunctionSymbol mainStub = new FunctionSymbol(new LabelSymbol("main"), new FunctionType(Type.INT));
	...
	/**
	 * generate code for unary neg operator a := - b. MIPS: neg $dest,$src
	 *
	 * @param a
	 *            the a
	 * @param b
	...
	private void genCodeMIPS(Inst3a inst) throws IOException {
		...
		switch (inst.getOp()) {
			...
			case ARG:
				/**
				 * Generate for an ARG instruction. If the arg. is a pointer or
				 * an integer We load the argument into a register, and then
				 * write it onto the parameter area of the stack frame. If it is
				 * a local array, we compute its address and write it into the
				 * frame. We keep track of which arg this is in the global
				 * variable "nextArgCallMIPS". We assume that ARG instructions
				 * are always followed by other ARG instructions or CALL
				 * instructions.
				 */
			...
			case RETURN:
				...
				// /------------------------
				// this part is needed for multiple return statement in a
				// function
			...
			case LABEL:
				 ...
				 * register descriptor, since control may arrive at this label
				 * from other points in the code.
				 */
				...
				if (beginFunction) {
					...
					regalloc.flushAll();
					out.println(inst.getA().getName3a() + ":");
				}
				break;
			...
			case VAR:
				...
				/*
				 * Allocate 4*n bytes for this variable to hold an integer (n=1)
				 * or an integer array (n = size) on the current top of stack
				 */
				if (inst.getA().isParam()) {
					inst.getA().setOffset(nextArgMIPS + stackSize);
					if (debugCG)
						System.out
								.println("\\ (param)" + inst.getA().getName3a() + "  @ = " + (nextArgMIPS + stackSize));
					nextArgMIPS += 4;
				} else
			...
			case BEGINFUNC:
				...
				// stack information
				nbMaxarg = CallerInformation.get(currentFunctionLabel);// max
																		// number
																		// of
																		// argument
																		// of
				...
				// Reservation of the arg build area (for compatibility with
				// nachos
				// and printf,scanf)
				if (nbMaxarg < 4)
					nbMaxarg = 4;

				sizeLocal = StackInformation.get(currentFunctionLabel); // number
																		// of
																		// local
																		// +
																		// temporaries
				stackSize = nbMaxarg * 4 + sizeLocal + 4 + 4; // +4 for the
																// $R_RET
																// save and +4
																// for
																// gp register
				...
				topOfStack = nbMaxarg * 4 + 4 + 4; // +4 for stack_register and
													// +4
													// for gp register
				...
				if (currentFunctionLabel.equals("main")) {
					...
					for (int i = 0; i < 8; i++)
						out.println(
								"\t sw	$" + (16 + i) + "," + (nbMaxarg * 4 + 4 + 4 + i * 4) + "($" + R_STACK + ")");
				}
	...
	private void genCodeMIPSBinaryOp(String inst, Operand3a dest, Operand3a op1, Operand3a op2) throws IOException {
		MIPSCGTriplet<MIPSRegDesc> registers = regalloc.allocateRegisters(dest, op1, op2);
	...
	private int tab_addressMIPS(Operand3a tab, Operand3a indice) {
		...
		if (tab.isPointer()) {
			...
			out.println(R_INDENT + "add   $" + r2 + ",$" + R_STACK + "," + offset); // r2=
																					// offset
																					// +
																					// R_stack
